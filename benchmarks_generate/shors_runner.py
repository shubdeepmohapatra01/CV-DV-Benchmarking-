...
def sample_p_and_estimate_period(p_dist, paxis, max_denominator=100):
    # Sanitize the input distribution: remove NaNs and negative/inf values
    p_dist = np.nan_to_num(p_dist, nan=0.0, posinf=0.0, neginf=0.0)
    p_dist = np.clip(p_dist, 0, None)  # Ensure all values are non-negative
    ...
    # Sample from the cleaned probability distribution
    ...
    # Only return r if j and r are coprime (i.e., gcd == 1)
