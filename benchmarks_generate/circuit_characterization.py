...
from features import (
    collect_cvcircuit_metrics, evaluate_quantum_metrics,max_energy_wigner_trunc_incremental
)
...
PERFORMANCE_KEYS = ['Truncation Cost', 'Wigner Negativity', 'Average Energy','Max Energy','Max Wigner Negativity','Max Truncation Cost']
...
def average_over_timesteps(circuit_template, U1, qmr, qbr, cutoff, steps, dt, num_qumodes, num_qubits, sample_every=5):
    trunc_costs, wigner_negs, energies,max_wigner_cost,max_energy_cost,max_trunc_cost = [], [], [], [], [], []
    ...
    for i, t in enumerate(np.arange(0, steps * dt, dt)):
        ...
        if i % sample_every == 0:
            ...
            max_energy, max_wigner, max_trunc, final_energy, final_wigner, final_trunc = max_energy_wigner_trunc_incremental(
            circuit, cutoff, num_qumodes=num_qumodes, num_qubits=num_qubits,
                           omega_qumode=1.0, omega_qubit=1.0
            )
            trunc_costs.append(final_trunc)
            wigner_negs.append(final_wigner)
            energies.append(final_energy)
            max_wigner_cost.append(max_wigner)
            max_energy_cost.append(max_energy)
            max_trunc_cost.append(max_trunc)

    return {
        "Truncation Cost": np.mean(trunc_costs),
        "Wigner Negativity": np.mean(wigner_negs),
        "Average Energy": np.mean(energies),
        "Max Energy": np.mean(max_energy_cost),
        "Max Wigner Negativity": np.mean(max_wigner_cost),
        "Max Truncation Cost": np.mean(max_trunc)
    }
...
def characterize_circuit(name, circuit, cutoff, num_qubits=1, num_qumodes=1, stateop=None,
    ...
    if stateop is not None:
        ...
        # Max metrics by incrementally adding gates
        max_energy, max_wigner, max_trunc, final_energy, final_wigner, final_trunc = max_energy_wigner_trunc_incremental(
            circuit, cutoff, num_qumodes=num_qumodes, num_qubits=num_qubits,
                           omega_qumode=1.0, omega_qubit=1.0
        )
        ...
        metrics.update({
            "Truncation Cost": final_trunc,
            "Wigner Negativity": final_wigner,
            "Average Energy": final_energy,
            "Max Energy": max_energy,
            "Max Wigner Negativity": max_wigner,
            "Max Truncation Cost": max_trunc
        })
...
def main():
    ...
    print("Circuit done")
    print(perf_all["StateTransferCVtoDV"])
    ...
    plot_radar_group(struct_all, STRUCTURAL_KEYS, os.path.join(OUTPUT_DIR, "summary_structural2.png"))
    plot_radar_group(perf_all, PERFORMANCE_KEYS, os.path.join(OUTPUT_DIR, "summary_quantum2.png"))
    
    output_file = os.path.join("circuit_characters", "benchmarks_data.json")
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    with open(output_file, "w") as f:
        json.dump(perf_all, f, indent=4)

    print(f"benchmark values saved to {output_file}")
